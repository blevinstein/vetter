...
//! AppKit:
//!
//! 1. [`crate::cards::spans::parse_ansi_spans`] reads `\x1b[<codes>m`
//!    runs and returns `Vec<AnsiSpan>` (plain text + accumulated
//!    style state). That half is pure Rust and lives in
//!    [`crate::cards::spans`], shared with every other approval
//!    surface; no `objc2` types touched.
...
//! popover calls. What remains here is stage 2 — the AppKit
//! translation of a [`SpanStyle`] into concrete `NSColor` / `NSFont`
//! values.
...
// Re-exported so `popover.rs` keeps calling `popover_attr::strip_ansi`
// and the span types stay nameable at this path after the lift into
// `crate::cards::spans`.
pub use crate::cards::spans::{strip_ansi, AnsiColor, AnsiSpan, SpanStyle};
